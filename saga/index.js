...
function Fetch(apiForm) {
    const { method = 'GET', path, params = {}, body, edges = false, version = 'v2', table = 'ims', headers, url, encodeUrl } = apiForm;
    let mergeParams = '';
    ...
    try {
        ...
        if (Array.isArray(apiForm)) {
            ...
            if (!data) {
                ...
                initData = data;
                if (initData && initData.status && initData.status !== 200) throw 'API Status Error';
                if (apiForm.path && apiForm.path.indexOf('/count') !== -1) initData = initData.datas.count;
            }
        }
                    ...
                    if (v.status === 200 && v.datas.length) {
                        ...
                        for (let key in v.datas) {
                            if (v.datas.hasOwnProperty(key)) {
                                newData[v.datas[key].data_bookmark_field] = v.datas[key].data_bookmark_name;
                            }
                        }
...
function* titleSaga(action) {
    ...
    try {
        ...
        if (!workflowSenderData) {
            ...
            if (!workflowSenderData.action_id || !workflowSenderData.workflow_sender_title) throw 'ERROR';
            else {
    ...
    try {
        if (queryStringParams.table_idx) {
            if (keyInUseArr.length) {
                const apiForm = {
                    method: 'POST',
                    url: `https://storage.uneedcomms.com/api/fm/v2/${CURRENT_USER.domain_id}/keyinuse`,
                    params: {
                        ...queryStringParams,
            ...
            if (!senderContentsData || !senderContentsData.content_idx) {
                ...
                let data = senderContentsData;
                workflow_data.ims_workflow_action.workflow[0].setting.sender.cms[queryStringParams.desc.senderCode] = {
